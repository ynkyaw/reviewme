...
function CreateJson()
{
    ...
    if(myvalidation360())
    {
        ...
        var jStr = "{\"itemreviews_id\": "+<?=$reviewid?>+",\"reviewerid\": "+<?=$reviewerid?>+","; 
        
        jStr+="\"products_id\": "+<?=$productid?>+",\"isdeleted\": false,\"created\": \""+datetime+"\",";
        jStr+="\"modified\": \""+datetime+"\",\"finish\":true,\"itemreviewsresultdetail\": [";
...
function sendJSON(jsonStr)
{
    $.ajax({
          	type: "POST",
          	url: "<?php echo Router::url('/', true);?>itemreviewsresults.json",
            data: jsonStr,
            ...
            {          
	            ...
	            if(result!='Failed')
	            {
	                window.location = '/reviewlist/indexproduct/'+<?= $reviewid ?>+'';                
	            }
...
<div id="page-wrapper">
    ...
    <div class="row">
        <div class="col-sm-10">
         ...
         if($projectName != null)
         {
            foreach ($projectName as $projectName):
            ?>
