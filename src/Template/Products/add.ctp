                    ...
                    <?= $this->Form->create($product,array('onsubmit' => 'return confirm("Are you sure you want to save?")')) ?>
                        <fieldset>
                            ...
                            <div class="form-group">
                                ...
                                <?= $this->Form->input('price',['class'=>'form-control','required','label'=>false,'required','onchange'=>'CheckPrice(this);']) ?>
                            </div>
                            <div class="form-group">
...
function CheckName(txtRankName)
{
     ...
     if(rankValue == "")
     {
        txtRankName.setCustomValidity("Product Name Required!");  
     }
     else if(rankValue.length<2)
     {
        txtRankName.setCustomValidity("Product Name length error!");
       
     }else if(!name_pattern.test(rankValue)&&!singleName.test(rankValue))
     {
        txtRankName.setCustomValidity("Product Title pattern mismatch!");
     }else {
...
function CheckModel(txtmodel)
{
     ...
     {
      txtmodel.setCustomValidity("Product Model Required!");  
     }
     else if(rankDescriptionValue.length<1)
     {
        txtmodel.setCustomValidity("Product Model length error!");
     }else {
...
function CheckPrice(txtprice)
{
     ...
     if(rankDescriptionValue == "")
     {
      txtprice.setCustomValidity("Product Price Required!");  
     }
     else if(rankDescriptionValue.length<1)
     {
        txtprice.setCustomValidity("Product Price length error!");
     }else {
...
function CheckDescription(txtdescription)
{
     ...
     if(rankDescriptionValue == "")
     {
      txtdescription.setCustomValidity("Product Description Required!");  
     }
     else if(rankDescriptionValue.length<1)
     {
        txtdescription.setCustomValidity("Product Description length error!");
     }else {
