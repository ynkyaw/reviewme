        ...
        <div class="panel panel-info" style="width:700px;">                                    
            <div id="groupA" class="panel-collapse collapse in">
                <div class="panel-heading">
                    ...
                    <?= $this->Form->create($service,array('onsubmit' => 'return confirm("Are you sure you want to save?")')) ?>
                        <fieldset>
                            <div class="form-group">
                                <label for="name" style="floating:right">Name</label><label style="color:red;font-size:18px;">*</label>
                                <?= $this->Form->input('servicename',['class'=>'form-control','label'=>false,'required','onchange'=>'CheckName(this);']) ?>
                            </div>
                            <div class="form-group">
                                <label for="description" style="floating:right">Description</label><label style="color:red;font-size:18px;">*</label>
                                <?= $this->Form->input('description',['class'=>'form-control','type' => 'textarea','label'=>false,'required','onchange'=>'CheckName(this);']) ?>
                            </div>
...
function CheckName(txtName)
{
     ...
     if(rankValue == "")
     {
        txtName.setCustomValidity("Service Name Required!");  
     }
     else if(rankValue.length<2)
     {
        txtName.setCustomValidity("Service Name length error!");
       
     }else if(!name_pattern.test(rankValue)&&!singleName.test(rankValue))
     {
        txtName.setCustomValidity("Service Title pattern mismatch!");
     }else {
...
function CheckDescription(txtDescription)
{
     ...
     {
      txtDescription.setCustomValidity("Service Description Required!");  
     }
     else if(rankDescriptionValue.length<1)
     {
        txtDescription.setCustomValidity("Service Description length error!");
     }else {
