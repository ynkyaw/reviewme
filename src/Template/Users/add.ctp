            ...
            <div id="groupA" class="panel-collapse collapse in">
                <div class="panel-heading">
                    ...
                    <fieldset>
                        ...
                        <div class="form-group">
                            <label for="en" style="floating:right"> Employee </label><label style="color:red;font-size:18px;">*</label><br />
                            <span id="txtHint2"></span>
                           
                           <?= $this->Form->input('employee_id',['options' => $employees,'empty' => 'Select Employee','class'=>'form-control','label'=>false,'id' => 'empname','onblur' => 'findEmployee()']) ?>
                        </div>
                    ...
                    <?= $this->Form->button('Save',['class'=>"btn btn-primary btn-sm",'id'=>'btnSave']) ?>
                    <?= $this->Form->end() ?>
                </div>
            </div>
        </div>
...
<script>
        function findUser()
        {
            ...
            if(userName.length>3)
            {
                $.ajax({
                    ...
                    {
                       $("#txtHint").html(data);
                       if(data.indexOf("User Already Exist!") != -1)
                        {
        ...
        function findEmployee() 
        {
            ...
            $.ajax({
                    type: "GET",
                    url:"<?php echo Router::url('/', true); ?>users/checkemployee/"+empname
            }).done(function(data)
            {
                $("#txtHint2").html(data);
                if(data.indexOf("Employee Already Exists!") != -1)
                {
