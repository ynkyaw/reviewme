...
<div id="page-wrapper">
    <div class="row">
        <div class="col-sm-10">
            <h4 class="page-header">Add Question Category</h4>
        </div>            
    </div>
                    ...
                    <?= $this->Form->create($questioncategory,array('onsubmit' => 'return confirm("Are you sure you want to save?")')) ?>
                        <fieldset>
                            ...
                            <div class="form-group">
                                ...
                                <?= $this->Form->input('questioncategoryname',['class'=>'form-control','label'=>false,'required','onchange'=>'CheckName(this)']) ?>
                            </div>
                            <div class="form-group">
                                <label for="description">Question Category Description</label>
                                <?= $this->Form->input('questioncategorydescription',['class'=>'form-control','type' => 'textarea','label'=>false]) ?>
                            </div>
                            <div class="form-group">
                                <label for="weight" style="floating:right">Question Category Weight</label><label style="color:red;font-size:18px;">*</label>
                                <?= $this->Form->input('questioncategoryweight',['class'=>'form-control','min' => 1,'max' => 5,'label'=>false,'required','onchange'=>'CheckWeight(this)']) ?>
                            </div>
...
<script type="text/javascript">
    ...
    function CheckName(txtQName)
    {
         ...
         {
          txtQName.setCustomValidity("QuestionCategory Name Required!");  
         }
         else if(rankDescriptionValue.length<1)
         {
            txtQName.setCustomValidity("QuestionCategory Name length error!");
         }else {
    ...
    function CheckWeight(txtWeight)
    {
         ...
         if(rankDescriptionValue == "")
         {
          txtWeight.setCustomValidity("QuestionCategory Weight Required!");  
         }
         else if(rankDescriptionValue.length<1)
         {
            txtWeight.setCustomValidity("QuestionCategory Weight length error!");
         }else {
